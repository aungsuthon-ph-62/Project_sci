<div class="pagetitle">
    <h1>ตรวจสอบและเผยแพร่ข่าว</h1>
    <nav>
        <ol class="breadcrumb">
            <li class="breadcrumb-item"><a href="index">หน้าหลัก</a></li>
            <li class="breadcrumb-item active">ตรวจสอบและเผยแพร่ข่าว</li>
        </ol>
    </nav>
</div><!-- End Page Title -->
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-body p-3 p-lg-5">
                    <?php
                    $news = new News;
                    $news_result = $news->get_draft($_GET['id']);
                    $row = $news_result[0];
                    ?>
                    <?php if ($row) { ?>
                        <form id="newsAddForm" action="private/app/controller/news.php" method="POST" enctype="multipart/form-data" autocomplete="off">
                            <input type="hidden" name="action" value="publicNews">
                            <input type="hidden" name="oldBanner" value="<?= $row['draft_banner'] ?>">
                            <input type="hidden" name="newsId" value="<?= $row['draft_unique'] ?>">
                            <input type="hidden" name="author" value="<?= $row['draft_author'] ?>">
                            
                            <div class="mb-3">
                                <label for="previewImg"><i class="fa-solid fa-image"></i> แนบรูปภาพหน้าปก</label>
                                <div class="p-lg-3">
                                    <img src="public/img/draft_banner/<?= $row['draft_banner'] ?>" id='previewImg' class='img-thumbnail rounded-lg w-25' alt="<?= $row['draft_banner'] ?>">
                                </div>
                            ...
                            <div class="mb-3">
                                ...
                                <input class="form-control" type="text" id="newsTopic" name="newsTopic" value="<?= $row['draft_topic'] ?>">
                            </div>
                            <div class="mb-3">
                            ...
                            <div class="mb-3">
                                ...
                                <textarea id="newsArticle" name="newsArticle" class="form-control"><?= $row['draft_article'] ?></textarea>
                            </div>

                            <button type="submit" class="btn btn-success rounded-pill float-start"><i class="bi bi-globe2"></i> เผยแพร่ข่าว</button>
                        </form>
                    <?php } ?>
                </div>
